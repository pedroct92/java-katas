...
public class RuleOrderBookProductImpl implements RuleOrder {

    @Override
    public void apply(ProcessedOrder processedOrder) {
        if (processedOrder.getOrder().getProduct().isBook()) {
            PackingSlip packingSlip = new PackingSlip();
            packingSlip.addProduct(processedOrder.getOrder().getProduct());

            processedOrder.setShipment(new Shipment(packingSlip, packingSlip));
        }
    }
}
